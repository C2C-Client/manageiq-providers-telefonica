class ManageIQ::Providers::Telefonica::InfraManager < ManageIQ::Providers::InfraManager
  ...
  def ensure_parent_provider
    ...
    # EmsTelefonicaInfra has associated a Provider. This relation will serve for relating EmsTelefonicaInfra
    # to possible many EmsTelefonicas deployed through EmsTelefonicaInfra

    # Name of the provider needs to be unique, get provider if there is one like that
    self.provider = ManageIQ::Providers::Telefonica::Provider.find_by(:name => name) unless provider

    attributes = {:name => name, :zone => zone}
    if provider
      provider.update_attributes!(attributes)
    else
      self.provider = ManageIQ::Providers::Telefonica::Provider.create!(attributes)
    end
  end
  ...
  def self.ems_type
    @ems_type ||= "telefonica_infra".freeze
  end
  ...
  def workflow_service
    telefonica_handle.detect_workflow_service
  end

  def register_and_configure_nodes(nodes_json)
    connection = telefonica_handle.detect_workflow_service
