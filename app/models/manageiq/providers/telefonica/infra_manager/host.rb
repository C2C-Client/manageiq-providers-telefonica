...
class ManageIQ::Providers::Telefonica::InfraManager::Host < ::Host
  ...
  def refresh_telefonica_services(ssu)
    ...
    telefonica_containerized_status = ssu.shell_exec(list_all_service_containers_cmd)

    services = MiqLinux::Utils.parse_telefonica_status(telefonica_status)
    if telefonica_containerized_status.present?
      containerized_services = MiqLinux::Utils.parse_telefonica_container_status(telefonica_containerized_status)
      services = MiqLinux::Utils.merge_telefonica_services(services, containerized_services)
    end
