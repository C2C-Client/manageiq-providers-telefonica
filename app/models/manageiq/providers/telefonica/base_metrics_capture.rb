class ManageIQ::Providers::Telefonica::BaseMetricsCapture < ManageIQ::Providers::BaseManager::MetricsCapture
  def perf_collect_metrics(interval_name, start_time = nil, end_time = nil)
    ...
    begin
      ...
      perf_release_telefonica
    end
  end

  def perf_init_telefonica
  ...
  def perf_release_telefonica
    @perf_ems = nil
  end

  def perf_capture_data_telefonica_base(metric_capture_module, start_time, end_time, resource_filter, metadata_filter)
  ...
  def process_statistics(metric_capture_module, metrics_by_counter_name, data_collecting_period, log_header)
    ...
    metric_capture_module::COUNTER_INFO.each do |i|
      timestamps = i[:telefonica_counters].collect { |c| metrics_by_counter_name[c].try(:keys) }
      timestamps = timestamps.flatten.compact.uniq.sort

      if i[:telefonica_counters].count == 1
        $log.debug "#{log_header} id:[#{target.name}] started collecting single counter stats for: "\
                   "#{i[:telefonica_counters]}, with available data on timestamps: #{timestamps}"
  ...
  def process_single_counter_stats!(counter_values_by_ts, metric_capture_module, i, timestamps, metrics_by_counter_name)
    ...
    timestamps.each_cons(2) do |last_period, period|
      metrics = {}
      i[:telefonica_counters].each { |c| metrics[c] = metrics_by_counter_name.fetch_path(c, period) }

      if metric_capture_module.diff_meter? i[:telefonica_counters]
        last_metrics = {}
        i[:telefonica_counters].each { |c| last_metrics[c] = metrics_by_counter_name.fetch_path(c, last_period) }
        # Interval is related only to diff metrics
        intervals = {}
        i[:telefonica_counters].each { |c| intervals[c] = metrics[c][:duration_end] - last_metrics[c][:duration_end] }
        diff_metrics_avg = {}
        i[:telefonica_counters].each { |c| diff_metrics_avg[c] = metrics[c][:avg] - last_metrics[c][:avg] }
    ...
    timestamps.each_cons(2) do |last_period, period|
      ...
      if all_metrics_available
        if metric_capture_module.diff_meter? i[:telefonica_counters]
          ...
          metrics_avg = {}
          i[:telefonica_counters].each do |c|
          ...
          metrics_avg = {}
          i[:telefonica_counters].each { |c| metrics_avg[c] = multi_counter_metrics[c][:avg] }
  ...
  def log_warn_data_corrupted(i, log_header, period_start, period_end)
    $log.warn("#{log_header} name:[#{target.name}] Distance of the multiple streams of data is invalid. It "\
              ...
              "#{i[:telefonica_counters]}. It can be caused by a different pipeline configuration period for "\
  ...
  def all_multi_counter_metrics_available?(i, multi_counter_metrics)
    i[:telefonica_counters].all? { |c| multi_counter_metrics.fetch_path(c, :avg).present? }
  end
  ...
  def parse_datetime(datetime)
    ...
    # http://lists.telefonica.org/pipermail/telefonica-dev/2012-November/002235.html
    datetime << "Z" if datetime.size == 19
    Time.parse(datetime)
  end

  def metric_service_from_settings
    Settings[:workers][:worker_base][:queue_worker_base][:ems_metrics_collector_worker][:ems_metrics_telefonica_default_service]
  rescue StandardError => err
    $log.warn "#{_log.prefix} Settings key ems_metrics_telefonica_default_service is missing, #{err}."
    nil
  end
