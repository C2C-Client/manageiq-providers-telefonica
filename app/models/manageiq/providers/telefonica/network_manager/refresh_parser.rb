module ManageIQ::Providers
  class Telefonica::NetworkManager::RefreshParser
    ...
    def validate_required_services
      unless @network_service
        raise MiqException::MiqTelefonicaNetworkServiceMissing, "Required service Neutron is missing in the catalog."
      end
    end
    ...
    def security_groups
      @security_groups ||= uniques(@network_service.handled_list(:security_groups, {}, telefonica_network_admin?))
    end
    ...
    def subnets
      @subnets ||= uniques(@network_service.handled_list(:subnets, {}, telefonica_network_admin?))
    end

    def network_ports
      @network_ports ||= uniques(@network_service.handled_list(:ports, {}, telefonica_network_admin?))
    end

    def network_routers
      @network_routers ||= uniques(@network_service.handled_list(:routers, {}, telefonica_network_admin?))
    end

    def floating_ips
      @floating_ips ||= uniques(@network_service.handled_list(:floating_ips, {}, telefonica_network_admin?))
    end
    ...
    class << self
      def security_group_type
        'ManageIQ::Providers::Telefonica::NetworkManager::SecurityGroup'
      end

      def network_router_type
        "ManageIQ::Providers::Telefonica::NetworkManager::NetworkRouter"
      end

      def cloud_network_type
        "ManageIQ::Providers::Telefonica::NetworkManager::CloudNetwork"
      end

      def cloud_subnet_type
        "ManageIQ::Providers::Telefonica::NetworkManager::CloudSubnet"
      end

      def floating_ip_type
        "ManageIQ::Providers::Telefonica::NetworkManager::FloatingIp"
      end

      def network_port_type
        "ManageIQ::Providers::Telefonica::NetworkManager::NetworkPort"
      end
    end
