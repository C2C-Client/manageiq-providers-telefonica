class ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager < ManageIQ::Providers::Telefonica::Inventory::Collector
  ...
  def cloud_services
    ...
    @cloud_services = compute_service.handled_list(:services, {}, telefonica_admin?)
  end
  ...
  def key_pairs
    ...
    @key_pairs = compute_service.handled_list(:key_pairs, {}, telefonica_admin?)
  end
  ...
  def vms
    ...
    @vms = compute_service.handled_list(:servers, {}, telefonica_admin?)
  end
  ...
  def tenants
    ...
    @tenants = manager.telefonica_handle.tenants.select do |t|
      # avoid 401 Unauth errors when checking for accessible tenants
      # the "services" tenant is a special tenant in telefonica reserved
  ...
  def vnfs
    ...
    @vnfs = nfv_service.handled_list(:vnfs, {}, telefonica_admin?)
  end

  def vnfds
    return [] unless nfv_service
    return @vnfds if @vnfds.any?
    @vnfds = nfv_service.handled_list(:vnfds, {}, telefonica_admin?)
  end
  ...
  def volume_templates
    ...
    @volume_templates = volume_service.handled_list(:volumes, {:status => "available"}, telefonica_admin?)
  end
  ...
  def volumes_by_id
    ...
    @volumes_by_id ||= volume_service.handled_list(:volumes, {}, telefonica_admin?).index_by(&:id)
  end

  def volume_snapshot_templates
    return [] unless volume_service
    return @volume_snapshot_templates if @volume_snapshot_templates.any?
    @volume_snapshot_templates = volume_service.handled_list(:list_snapshots_detailed, {:status => "available", :__request_body_index => "snapshots"}, telefonica_admin?).select do |s|
