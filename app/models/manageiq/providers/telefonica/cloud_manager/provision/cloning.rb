module ManageIQ::Providers::Telefonica::CloudManager::Provision::Cloning
  ...
  def do_clone_task_check(clone_task_ref)
    ...
    source.with_provider_connection(connection_options) do |telefonica|
      instance = if connection_options
                   telefonica.servers.get(clone_task_ref)
                 else
                   telefonica.handled_list(:servers).detect { |s| s.id == clone_task_ref }
                 end
  ...
  def prepare_for_clone_task
    #C2C Removed availability_zone parameter to avoid "No Availability Zones"
    clone_options = super
  ...
  def start_clone(clone_options)
    connection_options = {:tenant_name => cloud_tenant.try(:name)}
    if source.kind_of?(ManageIQ::Providers::Telefonica::CloudManager::VolumeTemplate)
    ...
    elsif source.kind_of?(ManageIQ::Providers::Telefonica::CloudManager::VolumeSnapshotTemplate)
      # remove the image_ref parameter from the options since it actually refers
      # to a volume, and then overwrite the default root volume with the volume
      # we are trying to boot the instance from
      clone_options.delete(:image_ref)
