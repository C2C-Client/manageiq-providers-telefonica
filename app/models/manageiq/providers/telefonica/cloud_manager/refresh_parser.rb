module ManageIQ::Providers
  class Telefonica::CloudManager::RefreshParser < ManageIQ::Providers::CloudManager::RefreshParser
    ...
    include ManageIQ::Providers::Telefonica::RefreshParserCommon::Objects
    include ManageIQ::Providers::Telefonica::RefreshParserCommon::Flavors
    include ManageIQ::Providers::Telefonica::RefreshParserCommon::OrchestrationStacks
    ...
    def validate_required_services
      unless @identity_service
        raise MiqException::MiqTelefonicaKeystoneServiceMissing, "Required service Keystone is missing in the catalog."
      end

      unless @compute_service
        raise MiqException::MiqTelefonicaNovaServiceMissing, "Required service Nova is missing in the catalog."
      end

      unless @image_service
        raise MiqException::MiqTelefonicaGlanceServiceMissing, "Required service Glance is missing in the catalog."
      end
    end
    ...
    def servers
      @servers ||= uniques(@connection.handled_list(:servers, {}, telefonica_admin?))
    end

    def vnfs
      @vnfs ||= uniques(@nfv_service.handled_list(:vnfs, {}, telefonica_admin?))
    end
    ...
    def get_availability_zones
      ...
      azs << nil # force the null availability zone for telefonica
    ...
    def get_key_pairs
      kps = uniques(@connection.handled_list(:key_pairs, {}, telefonica_admin?))
      process_collection(kps, :key_pairs) { |kp| parse_key_pair(kp) }
    end

    def get_servers
      telefonica_infra_hosts = @ems.provider.try(:infra_ems).try(:hosts)
      process_collection(servers, :vms) { |server| parse_server(server, telefonica_infra_hosts) }
    end

    def get_vnfds
      return unless @nfv_service

      unique_vnfds = uniques(@nfv_service.handled_list(:vnfds, {}, telefonica_admin?))
    ...
    def parse_availability_zone(az)
      if az.nil?
        uid        = "null_az"
        new_result = {
          :type    => "ManageIQ::Providers::Telefonica::CloudManager::AvailabilityZoneNull",
          :ems_ref => uid
        }
      else
        name = uid = az.zoneName
        new_result = {
          :type    => "ManageIQ::Providers::Telefonica::CloudManager::AvailabilityZone",
          :ems_ref => uid,
          :name    => name
        }
      end
    ...
    def parse_host_aggregate(ha)
      ...
      new_result = {
        :type     => "ManageIQ::Providers::Telefonica::CloudManager::HostAggregate",
        :ems_ref  => uid.to_s,
        :name     => ha.name,
    ...
    def parse_tenant(tenant)
      ...
      new_result = {
        :type        => "ManageIQ::Providers::Telefonica::CloudManager::CloudTenant",
        :name        => tenant.name,
    ...
    def flatten_quota(quota)
      ...
      quota.except("id", "tenant_id", "service_name", "metadata_items", "injected_file_content_bytes",
        ...
        end
        {
          ...
          :name         => key,
          :value        => value,
          :type         => "ManageIQ::Providers::Telefonica::CloudManager::CloudResourceQuota",
        }
      end
    end
    ...
    def self.key_pair_type
      'ManageIQ::Providers::Telefonica::CloudManager::AuthKeyPair'
    end
    ...
    def parse_server(server, parent_hosts = nil)
      ...
      if parent_hosts
        # Find associated host from TelefonicaInfra
      ...
      new_result = {
        :type                => "ManageIQ::Providers::Telefonica::CloudManager::Vm",
        :uid_ems             => uid,
        :ems_ref             => uid,
        :name                => server.name,
        :vendor              => "telefonica",
    ...
    def parse_vnf(vnf)
      ...
      new_result = {
        :type           => "ManageIQ::Providers::Telefonica::CloudManager::Vnf",
        :ems_ref        => uid,
        :name           => vnf.name,
    ...
    def parse_vnfd(vnfd)
      ...
      new_result = {
        :type        => "ManageIQ::Providers::Telefonica::CloudManager::VnfdTemplate",
        :ems_ref     => uid,
    ...
    def add_instance_disk(disks, size, location, name)
      super(disks, size, location, name, "telefonica")
    end

    def get_cloud_services
      source = 'compute'
      services = uniques(@compute_service.handled_list(:services, {}, telefonica_admin?))
