...
module TelefonicaHandle
  class Handle
    #C2C Added project_name for authentication
    attr_accessor :username, :password, :address, :port, :api_version, :security_protocol, :connection_options, :project_name
    ...
    def self.raw_connect_try_ssl(username, password, project_name, address, port, service = "Compute", options = nil,
      ...
      try_connection(security_protocol, ssl_options) do |scheme, connection_options|
        ...
        raw_connect(username, password, project_name, auth_url, service, opts)
      end
    end

    def self.raw_connect(username, password, project_name, auth_url, service = "Compute", extra_opts = nil)
      #C2C Added project_name parameter for authentication by using fog-telefonica api
     opts = {
        :provider                => 'TeleFonica',
    ...
    def self.auth_url(address, port = nil, scheme = "http", path = '')
      URI::Generic.build(:scheme => scheme, :host => address, :port => port, :path => path).to_s
    end
    ...
    #C2C set unnecessary parameter "port" to nil
    def initialize(username, password, address, port = nil, api_version = nil, security_protocol = nil,
      ...
      @thread_limit      = Settings.ems_refresh.telefonica.parallel_thread_limit || 0
    ...
    #C2C Added method for retrieving project_name
    def project_name
    ...
    def connect(options = {})
      ...
      svc_cache[tenant] ||= begin
        ...
        raw_service = self.class.raw_connect_try_ssl(username, password, project_name, address, port, service, opts,
                                                     security_protocol)

        service_wrapper_name = "#{service}Delegate"
        # Allow telefonica to define new services without explicitly requiring a
        # service wrapper.
        if TelefonicaHandle.const_defined?(service_wrapper_name)
          TelefonicaHandle.const_get(service_wrapper_name).new(raw_service, self, SERVICE_NAME_MAP[service])
        else
          raw_service
        end
      end
