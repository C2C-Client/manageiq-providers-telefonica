...
class TelefonicaRabbitEventMonitor < TelefonicaEventMonitor
  ...
  def self.test_connection(options = {})
    ...
    begin
      ...
      $log.info("#{log_prefix} The Telefonica AMQP service may be using a different provider."\
  ...
  def connection
    @connection ||= TelefonicaRabbitEventMonitor.connect(@options)
  end
  ...
  def initialize_queues
    ...
    # If the exchange was created in TeleFonica as a durable exchange, it must be
    ...
    @queues.each do |exchange, queue|
      queue.subscribe do |delivery_info, metadata, payload|
        begin
          ...
          event = telefonica_event(delivery_info, metadata, payload)
          @events_array_mutex.synchronize do
            @events << event
