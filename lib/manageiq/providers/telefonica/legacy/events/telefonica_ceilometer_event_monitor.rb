...
require 'manageiq/providers/telefonica/legacy/events/telefonica_event'
require 'manageiq/providers/telefonica/legacy/events/telefonica_ceilometer_event_converter'

class TelefonicaCeilometerEventMonitor < TelefonicaEventMonitor
  ...
  def self.available?(options = {})
    ...
    begin
      ...
      @panko = false
      $log.debug("Skipping Telefonica Panko events. Availability check failed with #{ex}. Trying Ceilometer.") if $log
  ...
  def self.event_service_settings
    Settings[:workers][:worker_base][:event_catcher][:event_catcher_telefonica_service]
  rescue StandardError => err
    $log.warn "#{_log.prefix} Settings key :event_catcher_telefonica_service is missing, #{err}."
    nil
  end
  ...
  def skip_history?
    Settings.fetch_path(:ems, :ems_telefonica, :event_handling, :event_skip_history) || false
  end
