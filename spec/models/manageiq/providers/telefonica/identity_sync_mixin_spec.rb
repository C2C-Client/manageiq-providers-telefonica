describe ManageIQ::Providers::Telefonica::IdentitySyncMixin do
  ...
  context "sync_users" do
    it "should create user, groups, and assign user to group" do
      ...
      cloud_tenant = FactoryBot.create(:cloud_tenant_telefonica, :name => 'project1', :ems_id => ems.id)
  ...
  context "create_or_find_miq_group_add_user" do
    it "should create new group and add user as member" do
      ...
      ems.miq_custom_set(ManageIQ::Providers::Telefonica::IdentitySyncMixin::IDENTITY_SYNC_ADMIN_ROLE_ID_NEW, admin_role.id)
      ems.miq_custom_set(ManageIQ::Providers::Telefonica::IdentitySyncMixin::IDENTITY_SYNC_MEMBER_ROLE_ID_NEW, member_role.id)
    ...
    it "should remove group membership if user is removed from project in TeleFonica" do
    end
  end
