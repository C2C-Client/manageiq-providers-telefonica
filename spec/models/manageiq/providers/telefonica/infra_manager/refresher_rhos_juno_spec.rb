...
describe ManageIQ::Providers::Telefonica::InfraManager::Refresher do
  before(:each) do
    ...
    @ems = FactoryBot.create(:ems_telefonica_infra, :zone => zone, :hostname => "192.168.24.1",
  ...
  it "will verify maintenance mode" do
    ...
    VCR.use_cassette("#{described_class.name.underscore}_rhos_juno_maintenance",
      ...
      @host = ManageIQ::Providers::Telefonica::InfraManager::Host.all.order(:ems_ref).detect { |x| x.name.include?('(NovaCompute)') }
  ...
  def assert_specific_host
    @host = ManageIQ::Providers::Telefonica::InfraManager::Host.all.detect { |x| x.name.include?('(Controller)') }
    ...
    expect(@host.private_networks.first).to be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::CloudNetwork::Private)
    expect(@host.network_ports.count).to    be > 0
    expect(@host.network_ports.first).to    be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::NetworkPort)
  ...
  def assert_specific_template(name, is_public = false)
    template = ManageIQ::Providers::Telefonica::InfraManager::Template.where(:name => name).first
