describe ManageIQ::Providers::Telefonica::InfraManager::MetricsCapture do
  require ManageIQ::Providers::Telefonica::Engine.root.join('spec/tools/telefonica_data/telefonica_data_test_helper').to_s
  ...
  before :each do
    ...
    allow(@metering).to receive(:list_meters).and_return(
      TelefonicaApiResult.new(@mock_meter_list.list_meters("metadata_counters")))

    @ems_telefonica = FactoryBot.create(:ems_telefonica_infra, :zone => @zone)
    allow(@ems_telefonica).to receive(:connect).with(:service => "Metering").and_return(@metering)

    @host = FactoryBot.create(:host_telefonica_infra, :ext_management_system => @ems_telefonica)
  end

  context "with standard interval data" do
    before :each do
      allow(@metering).to receive(:get_statistics) do |name, _options|
        TelefonicaApiResult.new(@mock_stats_data.get_statistics(name))
      end
    end

    it "treats telefonica timestamp as UTC" do
  ...
  context "with irregular interval data" do
    before do
      allow(@metering).to receive(:get_statistics) do |name, _options|
        TelefonicaApiResult.new(@mock_stats_data.get_statistics(name, "irregular_interval"))
      end
    ...
    def preload_data
      ...
      # spec/tools/telefonica_data/telefonica_perf_data/irregular_interval.yml
    ...
    #
    # the first two telefonica metrics should look like:
  ...
  context "memory utilization calculation" do
    it "cached and buffer memory should not count towards utilized memory as it can be freed" do
      ...
      # See https://review.telefonica.org/#/c/157257
