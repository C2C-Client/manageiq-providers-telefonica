describe ManageIQ::Providers::Telefonica::InfraManager::Host do
  describe "#refresh_telefonica_services" do
    let(:telefonica_status_text) do
      <<~EOT
        telefonica-nova-api:                     active
        telefonica-nova-cert:                    inactive  (disabled on boot)
        telefonica-nova-compute:                 active
        telefonica-nova-network:                 inactive  (disabled on boot)
        telefonica-nova-scheduler:               active
        telefonica-nova-conductor:               active
        telefonica-glance-api:                   active
        telefonica-glance-registry:              active
        telefonica-glance-for-test:              active
        telefonica-keystone:                     active
      EOT
    end

    let(:telefonica_containers_status_text) do
      <<~EOT
    ...
    let(:ssu) do
      double('ssu').tap do |ssu|
        expect(ssu).to receive(:shell_exec).with(/systemctl/).and_return(telefonica_status_text)
        expect(ssu).to receive(:shell_exec).with(/docker/).and_return(telefonica_containers_status_text)
      end
    end

    let(:host) do
      FactoryBot.create(:host_telefonica_infra).tap do |host|
        ...
        # create TeleFonica related SystemService to test against
        host.system_services << FactoryBot.create(:system_service, :name => 'telefonica-nova-api')
        # create generic Filesystem
        host.filesystems << FactoryBot.create(:filesystem)
        # create TeleFonica related Filesystems to test against
        ...
        # create HostServiceGroupTelefonica existing prior to calling HostTelefonicaInfra#refresh_telefonica_services
        glance = FactoryBot.create(:host_service_group_telefonica, :name => 'Glance services')
        # create glance_api and add it to both Host and HostServiceGroupTelefonica objects
        glance_api = FactoryBot.create(:system_service, :name => 'telefonica-glance-api')
        glance.system_services << glance_api
        host.system_services << glance_api
        # create glance_registry and add it to Host object only
        glance_registry = FactoryBot.create(:system_service, :name => 'telefonica-glance-registry')
        host.system_services << glance_registry

        # create glance_conf and add it both Host and HostServiceGroupTelefonica objects
        ...
        host.host_service_group_telefonicas << glance

        # create keystone system service on host
        host.system_services << FactoryBot.create(:system_service, :name => 'telefonica-keystone')
      end
    end

    before do
      FactoryBot.create(:host_service_group_telefonica, :host => host, :name => 'Keystone')
    end

    context "with stubbed MiqLinux::Utils" do
      it "makes proper utils calls" do
        miq_linux_utils_double = double('MiqLinux::Utils')
        allow(miq_linux_utils_double).to receive(:parse_telefonica_container_status).with(telefonica_containers_status_text).and_return([])
        allow(miq_linux_utils_double).to receive(:merge_telefonica_services).with([], []).and_return([])
        expect(miq_linux_utils_double).to receive(:parse_telefonica_status).with(telefonica_status_text).and_return([])
        expect(miq_linux_utils_double).to receive(:parse_telefonica_container_status).with(telefonica_containers_status_text).and_return([])
        stub_const('MiqLinux::Utils', miq_linux_utils_double)
        host.refresh_telefonica_services(ssu)
      end
    end
    ...
    describe "host_service_group_telefonicas names" do
      subject do
        host.refresh_telefonica_services(ssu)
        host.host_service_group_telefonicas.map(&:name)
      end
    ...
    describe "system_services names" do
      ...
      let(:expected) do
        [
          'telefonica-nova-api',
          'telefonica-glance-api',
          'telefonica-glance-registry',
          'telefonica-keystone',
        ]
      end
      ...
      let(:unexpected) do
        [
          'telefonica-nova-compute', # was not present in Host#system_services
        ]
      end
    ...
    describe "existing HostServiceGroupTelefonica gets updated" do
      let(:glance_host_service_group) do
        host.refresh_telefonica_services(ssu)
        host.host_service_group_telefonicas.where(:name => 'Glance').first
      end
      ...
      describe "system_service names" do
        ...
        let(:unexpected) do
          [
            'telefonica-nova-compute',
            'telefonica-keystone',
            'telefonica-glance-for-test',
          ]
        end
    ...
    it "creates association with existing SystemServices" do
      host.refresh_telefonica_services(ssu)
      # we test if SystemServices with associated HostServiceGroupTelefonicas from current Host
      # are included among all SystemServices of that host
      expect(host.system_services).to include(*(host.host_service_group_telefonicas.flat_map(&:system_services)))
    end

    it "creates association with existing Filesystems" do
      host.refresh_telefonica_services(ssu)
      # we test if Filesystems with associated HostServiceGroupTelefonicas from current Host
      # are included among all Filesystems of that host
      expect(host.filesystems).to include(*(host.host_service_group_telefonicas.flat_map(&:filesystems)))
    end
  end
  ...
  describe "Overriden auth methods for ssh fleecing," do
    let(:ext_management_system) do
      ...
      FactoryBot.create(:ems_telefonica_infra, :zone => zone).tap do |ems|
  ...
  describe "ironic tasks" do
    let(:ext_management_system) do
      _guid, _server, zone = EvmSpecHelper.create_guid_miq_server_zone
      FactoryBot.create(:ems_telefonica_infra, :zone => zone)
    end
    ...
    it "check task executes and queues refresh if success" do
      ...
      telefonica_handle = double("telefonica handle")
      expect(telefonica_handle).to receive(:detect_workflow_service).at_least(1).times { workflow_service }
      expect(telefonica_handle).to receive(:detect_baremetal_service) { baremetal_service }
      allow(ext_management_system).to receive(:telefonica_handle).and_return(telefonica_handle)
  ...
  describe 'after create callback' do
    let(:ext_management_system) { FactoryBot.create(:ems_telefonica_infra) }

    it "assigns scale out event fot host" do
      payload = { "instance_id" => "telefonica-perf-host-nova-instance" }
      ...
      event_hash = {
        ...
        :host_name  => "telefonica-perf-host-nova-instance"
      }

      EmsEvent.add(ext_management_system.id, event_hash)
      host = FactoryBot.create(:host_telefonica_infra, :ext_management_system => ext_management_system)
