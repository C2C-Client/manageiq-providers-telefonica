describe ManageIQ::Providers::Telefonica::InfraManager do
  ...
  context "verifying SSH keypair credentials" do
    it "verifies Telefonica SSH credentials successfully when all hosts report that the credentials are valid" do
      @ems = FactoryBot.create(:ems_telefonica_infra_with_authentication)
      FactoryBot.create(:host_telefonica_infra, :ext_management_system => @ems, :state => "on")
    ...
    it "fails to verify Telefonica SSH credentials when any hosts report that the credentials are invalid" do
      @ems = FactoryBot.create(:ems_telefonica_infra_with_authentication)
      host = FactoryBot.create(:host_telefonica_infra, :ext_management_system => @ems, :state => "on")
    ...
    it "disregards powered off hosts when verifying Telefonica SSH credentials" do
      @ems = FactoryBot.create(:ems_telefonica_infra_with_authentication)
      FactoryBot.create(:host_telefonica_infra, :ext_management_system => @ems, :state => "off")
      allow_any_instance_of(ManageIQ::Providers::Telefonica::InfraManager::Host).to receive(:verify_credentials).and_return(false)
      expect(@ems.send(:verify_ssh_keypair_credentials, nil)).to be_truthy
    end
    ...
    it "disregards host with no ems_cluster" do
      @ems = FactoryBot.create(:ems_telefonica_infra_with_authentication)
      FactoryBot.create(:host_telefonica_infra, :ext_management_system => @ems, :state => "on", :ems_cluster => nil)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::InfraManager::Host).to receive(:verify_credentials).and_return(false)
      expect(@ems.send(:verify_ssh_keypair_credentials, nil)).to be_truthy
    end
  end
  ...
  context "provider hooks" do
    ...
    it "related EmsTelefonica nullifies relation to ProviderTelefonica on EmsTelefonicaInfra destroy" do
      ...
      @ems_cloud = FactoryBot.create(:ems_telefonica_with_authentication)
  ...
  context "cloud disk usage" do
    before do
      @provider = FactoryBot.create(:provider_telefonica, :name => "undercloud")
      @cloud = FactoryBot.create(:ems_telefonica, :name => "overcloud", :provider => @provider)
      @infra = FactoryBot.create(:ems_telefonica_infra_with_stack, :name => "undercloud", :provider => @provider)
      @cluster = FactoryBot.create(:ems_cluster_telefonica, :ext_management_system => @infra)
    end

    it "Block Storage / Cinder" do
      expect(@cluster.cloud_block_storage_disk_usage).to eq(0)

      @cloud.cloud_volumes << FactoryBot.create(:cloud_volume_telefonica, :size => 11, :status => "noterror")
      @cloud.cloud_volumes << FactoryBot.create(:cloud_volume_telefonica, :size => 50, :status => "error")

      expect(@cluster.cloud_block_storage_disk_usage).to eq(11)
    end

    it "Object Storage / Swift" do
      stack = FactoryBot.create(:orchestration_stack_telefonica_infra, :name => "overcloud")
      stack.parameters << FactoryBot.create(:orchestration_stack_parameter_telefonica_infra, :name => "SwiftReplicas", :value => 3)
      stack.parameters << FactoryBot.create(:orchestration_stack_parameter_telefonica_infra, :name => "ObjectStorageCount", :value => 2)
  ...
  context "catalog types" do
    let(:ems) { FactoryBot.create(:ems_telefonica_infra_with_authentication) }
  ...
  let(:telefonica_infra_manager) { FactoryBot.create(:ems_telefonica_infra_with_authentication) }
