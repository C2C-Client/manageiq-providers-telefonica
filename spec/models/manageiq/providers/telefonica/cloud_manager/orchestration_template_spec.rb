describe ManageIQ::Providers::Telefonica::CloudManager::OrchestrationTemplate do
  describe ".eligible_manager_types" do
    it "lists the classes of eligible managers" do
      described_class.eligible_manager_types.each do |klass|
        expect(klass <= ManageIQ::Providers::Telefonica::CloudManager).to be_truthy
      end
    end
  end

  let(:yaml_template) { FactoryBot.create(:orchestration_template_telefonica_in_yaml) }
  let(:json_template) { FactoryBot.create(:orchestration_template_telefonica_in_json) }
