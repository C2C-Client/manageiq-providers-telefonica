describe ManageIQ::Providers::Telefonica::CloudManager::Vm do
  ...
  let(:terminated_vm) { FactoryBot.create(:vm_telefonica) }
  ...
  context "#is_available?" do
    let(:ems) { FactoryBot.create(:ems_telefonica) }
    let(:vm)  { FactoryBot.create(:vm_telefonica, :ext_management_system => ems) }
  ...
  context "when detroyed" do
    ...
    let(:provider_object) do
      double("vm_telefonica_provider_object", :destroy => nil).as_null_object
    end
    let(:vm)  { FactoryBot.create(:vm_telefonica, :ext_management_system => ems) }
  ...
  context "when resized" do
    ...
    let(:vm) { FactoryBot.create(:vm_telefonica, :ext_management_system => ems, :cloud_tenant => cloud_tenant) }
    let(:flavor) { FactoryBot.create(:flavor_telefonica, :ems_ref => '2') }
