describe ManageIQ::Providers::Telefonica::CloudManager::EventCatcher do
  before do
    @ems = FactoryBot.create(:ems_telefonica)
    allow(@ems).to receive(:authentication_status_ok?).and_return(true)
    allow(ManageIQ::Providers::Telefonica::CloudManager::EventCatcher).to receive(:all_ems_in_zone).and_return([@ems])
  end
  ...
  it "logs info about EMS that do not have Event Monitors available" do
    ...
    expect(ManageIQ::Providers::Telefonica::CloudManager::EventCatcher.all_valid_ems_in_zone).not_to include @ems
  end
  ...
  it "does not log info about unavailable Event Monitors when EMS can provide an event monitor" do
    ...
    expect(ManageIQ::Providers::Telefonica::CloudManager::EventCatcher.all_valid_ems_in_zone).to include @ems
  end
end
