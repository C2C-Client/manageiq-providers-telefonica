...
require Rails.root.join('spec/tools/environment_builders/telefonica/services/compute/data').to_s
require Rails.root.join('spec/tools/environment_builders/telefonica/services/identity/data/keystone_v2')
require Rails.root.join('spec/tools/environment_builders/telefonica/services/identity/data/keystone_v3')
require Rails.root.join('spec/tools/environment_builders/telefonica/services/image/data')
require Rails.root.join('spec/tools/environment_builders/telefonica/services/network/data/neutron')
require Rails.root.join('spec/tools/environment_builders/telefonica/services/network/data/nova')
require Rails.root.join('spec/tools/environment_builders/telefonica/services/orchestration/data')
require Rails.root.join('spec/tools/environment_builders/telefonica/services/storage/data')
require Rails.root.join('spec/tools/environment_builders/telefonica/services/volume/data')
...
module Telefonica
  module RefreshSpecCommon
    ...
    def assert_with_errors
      ...
      expect(ManageIQ::Providers::Telefonica::CloudManager::AuthKeyPair.count).to eq 0
    ...
    def snapshots_count
      if ::Settings.ems_refresh.try(:telefonica).try(:inventory_object_refresh)
    ...
    def assert_table_counts
      ...
      expect(FloatingIp.count).to                        eq network_data.floating_ips.sum
      expect(ManageIQ::Providers::Telefonica::CloudManager::AuthKeyPair.count).to eq compute_data.key_pairs.count
      expect(security_groups_without_defaults.count).to  eq security_groups_count
      expect(firewall_without_defaults.count).to         eq firewall_rules_count
      expect(CloudNetwork.count).to                      eq network_data.networks.count
      expect(CloudSubnet.count).to                       eq network_data.subnets.count
    ...
    def assert_ems
      ...
      if neutron_networking?
        expect(@ems.public_networks.first).to  be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::CloudNetwork::Public)
        expect(@ems.private_networks.first).to be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::CloudNetwork::Private)
      end
    end
    ...
    def assert_flavors
      ...
      assert_objects_with_hashes(ManageIQ::Providers::Telefonica::CloudManager::Flavor.all,
      ...
      ManageIQ::Providers::Telefonica::CloudManager::Flavor.all.each do |flavor|
    ...
    def assert_public_flavor_tenant_mapping
      @other_flavors = ManageIQ::Providers::Telefonica::CloudManager::Flavor.where(:publicly_available => true)
    ...
    def assert_private_flavor_tenant_mapping
      @private_flavor = ManageIQ::Providers::Telefonica::CloudManager::Flavor.where(:publicly_available => false).first
      expect(@private_flavor.cloud_tenants.length).to eq 1
    end

    def assert_specific_az
      # This tests TeleFonica functionality more than ManageIQ
      @nova_az = ManageIQ::Providers::Telefonica::CloudManager::AvailabilityZone.where(
        :type => ManageIQ::Providers::Telefonica::CloudManager::AvailabilityZone.name, :ems_id => @ems.id
      ).first
      # standard telefonica AZs have their ems_ref set to their name ("nova" in the test case)...
      # the "null" telefonica AZ has a unique ems_ref and name
    ...
    def assert_availability_zone_null
      # This tests TeleFonica functionality more than ManageIQ
      @az_null = ManageIQ::Providers::Telefonica::CloudManager::AvailabilityZoneNull.where(:ems_id => @ems.id).first
    ...
    def assert_key_pairs
      assert_objects_with_hashes(ManageIQ::Providers::Telefonica::CloudManager::AuthKeyPair.all, compute_data.key_pairs)
    end
    ...
    def assert_specific_networks
      ...
      specific_networks.each do |network|
        expect(network.cloud_tenant).to          be_kind_of(ManageIQ::Providers::Telefonica::CloudManager::CloudTenant)
        expect(network.ext_management_system).to be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager)
        expect(network.cloud_subnets.count).to   be > 0
        expect(network.cloud_subnets.first).to   be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::CloudSubnet)

        if network.external_facing?
          # It's a public network
          expect(network.public_network_vms.count).to be > 0
          expect(network.public_network_vms.first).to be_kind_of(ManageIQ::Providers::Telefonica::CloudManager::Vm)
          expect(network.public_network_routers.count).to be > 0
          expect(network.public_network_routers.first).to be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::NetworkRouter)
          expect(network).to be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::CloudNetwork::Public)
          expect(network.private_networks.count).to be > 0
          expect(network.floating_ips.count).to     be > 0
          expect(network.private_networks.first).to be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::CloudNetwork::Private)
          ...
          expect(network.vms.first).to be_kind_of(ManageIQ::Providers::Telefonica::CloudManager::Vm)
          expect(network.network_routers.count).to be > 0
          expect(network.network_routers.first).to be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::NetworkRouter)
          expect(network).to be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::CloudNetwork::Private)
          expect(network.public_networks.count).to be > 0
          expect(network.public_networks.first).to be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::CloudNetwork::Public)
        end
      end
    end
    ...
    def assert_specific_routers
      ...
      specific_routers.each do |network_router|
        ...
        expect(network_router.floating_ips.first).to   be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::FloatingIp)
        expect(network_router.network_ports.count).to  be > 0
        expect(network_router.network_ports.first).to  be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::NetworkPort)
        expect(network_router.cloud_networks.count).to be > 0
        expect(network_router.cloud_networks.first).to be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::CloudNetwork::Private)
        expect(network_router.cloud_networks).to match_array network_router.private_networks
        expect(network_router.cloud_network).to        be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::CloudNetwork::Public)
        expect(network_router.cloud_network).to        be == network_router.public_network
        expect(network_router.vms.first).to            be_kind_of(ManageIQ::Providers::Telefonica::CloudManager::Vm)
    ...
    def assert_templates
      ...
      templates = ManageIQ::Providers::Telefonica::CloudManager::Template.all.reject { |x| x.name.include?("-shelved") }
    ...
    def assert_specific_templates
      specific_templates = ManageIQ::Providers::Telefonica::CloudManager::Template.all.select do |x|
    ...
    def assert_specific_vms
      all_vms = ManageIQ::Providers::Telefonica::CloudManager::Vm.all
      if orchestration_supported?
    ...
    def assert_specific_vm(vm_name, attributes)
      vm = ManageIQ::Providers::Telefonica::CloudManager::Vm.where(:name => vm_name).first
      ...
      expect(vm.availability_zone).to      be_kind_of(ManageIQ::Providers::Telefonica::CloudManager::AvailabilityZone)
      expect(vm.floating_ip).to            be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::FloatingIp)
      ...
      if neutron_networking?
        ...
        expect(vm.floating_ips.first).to    be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::FloatingIp)
        expect(vm.network_ports.count).to   be > 0
        expect(vm.network_ports.first).to   be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::NetworkPort)
        expect(vm.cloud_networks.count).to  be > 0
        expect(vm.cloud_networks).to match_array vm.private_networks
        expect(vm.cloud_networks.first).to  be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::CloudNetwork::Private)
        expect(vm.cloud_subnets.count).to   be > 0
        expect(vm.cloud_subnets.first).to   be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::CloudSubnet)
        expect(vm.network_routers.count).to be > 0
        expect(vm.network_routers.first).to be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::NetworkRouter)
        expect(vm.public_networks.count).to be > 0
        expect(vm.public_networks.first).to be_kind_of(ManageIQ::Providers::Telefonica::NetworkManager::CloudNetwork::Public)
    ...
    def assert_specific_template_created_from_vm
      @snap = ManageIQ::Providers::Telefonica::CloudManager::Template.where(
    ...
    def assert_specific_vm_created_from_snapshot_template
      t = ManageIQ::Providers::Telefonica::CloudManager::Vm.where(:name => "EmsRefreshSpec-PoweredOn-FromSnapshot").first
