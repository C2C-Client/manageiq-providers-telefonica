describe ManageIQ::Providers::Telefonica::CloudManager::Provision::VolumeAttachment do
  before do
    ...
    @template = FactoryBot.create(:template_telefonica, :ext_management_system => @ems)
    @flavor = FactoryBot.create(:flavor_telefonica)
    @volume = FactoryBot.create(:cloud_volume_telefonica)

    @task = FactoryBot.create(:miq_provision_telefonica,
                              :source  => @template,
                              :state   => 'pending',
                              :status  => 'Ok',
                              :options => {
  ...
  context "#configure_volumes" do
    it "create volumes" do
      ...
      expect(@task.create_requested_volumes(@task.options[:volumes])) == [default_volume, requested_volume]
    end
  end
  ...
  context "#check_volumes" do
    it "status pending" do
      ...
      service = double
      allow(service).to receive_message_chain('volumes.get').and_return FactoryBot.build(:cloud_volume_telefonica,
                                                                                         :status => "pending")
      allow(@task.source.ext_management_system).to receive(:with_provider_connection)\
        .with(:service => 'volume', :tenant_name => nil).and_yield(service)
