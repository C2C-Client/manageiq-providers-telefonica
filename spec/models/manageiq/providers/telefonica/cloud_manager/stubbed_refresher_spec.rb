require_relative '../telefonica_stubs'

describe ManageIQ::Providers::Telefonica::CloudManager::Refresher do
  include TelefonicaStubs

  describe "refresh" do
    before do
      @ems = FactoryBot.create(:ems_telefonica_with_authentication)
    ...
    context "targeted refresh workaround" do
      ...
      it "works around keypair targeted refresh by refreshing all keypairs without creating duplicates" do
        ...
        2.times do
          ...
          expect(ManageIQ::Providers::Telefonica::CloudManager::AuthKeyPair.count).to eq(test_counts(@data_scaling)[:key_pairs_count])
        end
      end
    ...
    def refresh_spec
      @ems.reload
      # with_telefonica_stubbed(stub_responses) do
    ...
    def assert_table_counts
      actual = {
        :auth_key_pair                 => ManageIQ::Providers::Telefonica::CloudManager::AuthKeyPair.count,
    ...
    def setup_mocked_collector
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:image_service).and_return(double(:name => "not-glance"))
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:availability_zones).and_return(mocked_availability_zones)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:availability_zones_compute).and_return(mocked_availability_zones)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:availability_zones_volume).and_return(mocked_availability_zones)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:cloud_services).and_return(mocked_cloud_services)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:flavors).and_return(mocked_flavors)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:host_aggregates).and_return(mocked_host_aggregates)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:images).and_return(mocked_miq_templates)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:key_pairs).and_return(mocked_key_pairs)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:quotas).and_return(mocked_quotas)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:quotas).and_return([])
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:quotas).and_return([])
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:orchestration_stacks).and_return(mocked_orchestration_stacks)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:vms).and_return(mocked_vms)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:vnfs).and_return(mocked_vnfs)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:vnfds).and_return(mocked_vnfds)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:tenants).and_return(mocked_cloud_tenants)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:private_flavor).and_return(nil)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:volume_templates).and_return(mocked_volume_templates)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::CloudManager).to receive(:volume_snapshot_templates).and_return(mocked_volume_snapshot_templates)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::TargetCollection).to receive(:volume_service).and_return(double(:name => "not-cinder"))
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::TargetCollection).to receive(:image_service).and_return(double(:name => "not-glance"))
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::TargetCollection).to receive(:orchestration_service).and_return(double(:name => "not-heat"))
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::TargetCollection).to receive(:network_service).and_return(double(:name => "not-neutron"))
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::TargetCollection).to receive(:cloud_volume_backups).and_return(mocked_cloud_volume_backups)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::TargetCollection).to receive(:key_pairs).and_return(mocked_key_pairs)
      allow_any_instance_of(ManageIQ::Providers::Telefonica::Inventory::Collector::TargetCollection).to receive(:cloud_subnets).and_return([])
    end
