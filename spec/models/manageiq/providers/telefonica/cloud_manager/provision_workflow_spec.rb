describe ManageIQ::Providers::Telefonica::CloudManager::ProvisionWorkflow do
  ...
  context "without applied tags" do
    ...
    context "security_groups" do
      context "non cloud network" do
        it "#get_targets_for_ems" do
          sg = FactoryBot.create(:security_group_telefonica, :ext_management_system => provider.network_manager)
      ...
      context "cloud network" do
        it "#get_targets_for_ems" do
          ...
          sg_cn = FactoryBot.create(:security_group_telefonica, :ext_management_system => provider.network_manager, :cloud_network => cn1)
    ...
    context "Instance Type (Flavor)" do
      it "#get_targets_for_ems" do
        flavor = FactoryBot.create(:flavor_telefonica)
  ...
  context "with applied tags" do
    ...
    before do
      ...
      2.times { FactoryBot.create(:security_group_telefonica, :ext_management_system => provider.network_manager) }
      ct1 = FactoryBot.create(:cloud_tenant)
      ct2 = FactoryBot.create(:cloud_tenant)
      provider.cloud_tenants << ct1
      provider.cloud_tenants << ct2
      provider.flavors << FactoryBot.create(:flavor_telefonica)
      provider.flavors << FactoryBot.create(:flavor_telefonica)
  ...
  context "With a user" do
    it "pass platform attributes to automate" do
      stub_dialog
      assert_automate_dialog_lookup(admin, 'cloud', 'telefonica')
    ...
    context "With a Valid Template" do
      ...
      context "#allowed_instance_types" do
        let(:template) { FactoryBot.create(:template_telefonica, :hardware => hardware, :ext_management_system => provider) }
        ...
        context "with regular hardware" do
          ...
          it "filters flavors too small" do
            flavor = FactoryBot.create(:flavor_telefonica, :memory => 1.gigabyte, :root_disk_size => 1.terabyte)
            provider.flavors << flavor
            provider.flavors << FactoryBot.create(:flavor_telefonica, :memory => 1.gigabyte, :root_disk_size => 1.megabyte) # Disk too small
            provider.flavors << FactoryBot.create(:flavor_telefonica, :memory => 1.megabyte, :root_disk_size => 1.terabyte) # Memory too small
      ...
      context "with valid relationships" do
        it "#allowed_availability_zones" do
          az = FactoryBot.create(:availability_zone_telefonica)
          az.provider_services_supported = ["compute"]
          excluded_az = FactoryBot.create(:availability_zone_telefonica)
        ...
        it "#allowed_availability_zones with NULL AZ" do
          ...
          az.provider_services_supported = ["compute"]
          provider.availability_zones << az
          provider.availability_zones << FactoryBot.create(:availability_zone_telefonica_null, :ems_ref => "null_az")
        ...
        it "#allowed_guest_access_key_pairs" do
          kp = ManageIQ::Providers::Telefonica::CloudManager::AuthKeyPair.create(:name => "auth_1")
          provider.key_pairs << kp
          expect(workflow.allowed_guest_access_key_pairs).to eq(kp.id => kp.name)
        end

        it "#allowed_security_groups" do
          sg = FactoryBot.create(:security_group_telefonica)
      ...
      context "availability_zone_to_cloud_network" do
        it "has one when it should" do
          subnet = FactoryBot.create(:cloud_subnet_telefonica)
          FactoryBot.create(:cloud_network_telefonica, :ext_management_system => provider.network_manager, :cloud_subnets => [subnet])

          expect(workflow.allowed_cloud_networks.size).to eq(1)
        end

        it "filters cloud networks without subnets" do
          FactoryBot.create(:cloud_network_telefonica, :ext_management_system => provider.network_manager)
      ...
      context "tenant filtering" do
        before do
          @ct1 = FactoryBot.create(:cloud_tenant_telefonica)
          @ct2 = FactoryBot.create(:cloud_tenant_telefonica)
          provider.cloud_tenants << @ct1
          provider.cloud_tenants << @ct2
        end
        ...
        context "cloud networks" do
          before do
            subnet1 = FactoryBot.create(:cloud_subnet_telefonica)
            subnet2 = FactoryBot.create(:cloud_subnet_telefonica)
            subnet3 = FactoryBot.create(:cloud_subnet_telefonica)
            subnet4 = FactoryBot.create(:cloud_subnet_telefonica)
            subnet5 = FactoryBot.create(:cloud_subnet_telefonica)
            @cn1 = FactoryBot.create(:cloud_network_private_telefonica,
                                      :cloud_tenant          => @ct1,
                                      :ext_management_system => provider.network_manager,
                                      :cloud_subnets         => [subnet1])
            @cn2 = FactoryBot.create(:cloud_network_private_telefonica,
                                      :cloud_tenant          => @ct2,
                                      :ext_management_system => provider.network_manager,
                                      :cloud_subnets         => [subnet2])
            @cn3 = FactoryBot.create(:cloud_network_public_telefonica,
                                      :cloud_tenant          => @ct2,
                                      :ext_management_system => provider.network_manager,
                                      :cloud_subnets         => [subnet3])

            @cn_shared        = FactoryBot.create(:cloud_network_private_telefonica,
                                                   :shared                => true,
                                                   :cloud_tenant          => @ct2,
                                                   :ext_management_system => provider.network_manager,
                                                   :cloud_subnets         => [subnet4])
            @cn_public_shared = FactoryBot.create(:cloud_network_public_telefonica,
                                                   :shared                => true,
                                                   :cloud_tenant          => @ct2,
                                                   :ext_management_system => provider.network_manager,
        ...
        context "security groups" do
          before do
            @sg1 = FactoryBot.create(:security_group_telefonica)
            @sg2 = FactoryBot.create(:security_group_telefonica)
        ...
        context "floating ip" do
          before do
            cloud_network_public   = FactoryBot.create(:cloud_network_public_telefonica)
            cloud_network_public_2 = FactoryBot.create(:cloud_network_public_telefonica)
            router                 = FactoryBot.create(:network_router_telefonica,
                                                        :cloud_network => cloud_network_public)
            @cloud_network         = FactoryBot.create(:cloud_network_private_telefonica,
                                                        :cloud_tenant => @ct2)
            @cloud_network_2       = FactoryBot.create(:cloud_network_private_telefonica,
                                                        :cloud_tenant => @ct2)
            _subnet                = FactoryBot.create(:cloud_subnet_telefonica,
