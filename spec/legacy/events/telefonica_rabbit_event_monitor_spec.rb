...
describe TelefonicaRabbitEventMonitor do
  before do
    @original_log = $log
    ...
    allow(TelefonicaRabbitEventMonitor).to receive(:connect).and_return(@rabbit_connection)
  end
  ...
  context "testing a connection" do
    it "returns true on a successful test" do
      ...
      expect(TelefonicaRabbitEventMonitor.test_connection(@options)).to be_truthy
    end
    ...
    it "return false on an unsuccessful test with bad credentials" do
      ...
      expect(@rabbit_connection).to receive(:close)
      expect(TelefonicaRabbitEventMonitor.test_connection(@options)).to eq(false)
    end
  ...
  it "#remove_legacy_queues (private)" do
    ...
    @event_monitor = TelefonicaRabbitEventMonitor.new(@options)
